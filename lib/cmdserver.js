...
CmdServer.prototype.begin = function () {
  var socket = this.argv['S'] || this.argv['socket'];
  console.dir(this.argv);

  client.error = function (err) {
    this.listen('what.socket');
  }.bind(this);

  client.attach('what.socket');
}
...
CmdServer.prototype.startRepl = function () {
  var rl, prefix, client, router, name, startRepl;
  if (this.repl) return;
  startRepl = this.startRepl.bind(this);
...
module.exports = {
  createServer: function createServer(argv) {
